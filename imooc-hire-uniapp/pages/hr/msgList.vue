...
<script>
	...
	export default {
		...
		onLoad(options) {
			...
			// 	console.log('msgList.vue 收到服务器内容：' + res.data);
			...
			uni.$on("receiveMsgInMsgListVue", function(data) {
				...
				me.dealReceiveMsgSlef(JSON.parse(data));
			});
		...
		methods: {
			getMsgList(page) {
				...
				var senderId = this.resumeUserId;
			...
			dealReceiveMsgSlef(msgJSON) {
					...
					if (senderId == this.meInfo.id && receiverId == this.resumeUserId) {
						console.log("进来了~~~");
					} else {
						return;
					}
			...
			clearMyUnReadCounts() {
				...
				var oppositeId = this.resumeUserId;
			...
			saveLastestMsgToLocal(msgContent, chatTime) {
				...
				for ( var i = 0 ; i < lastestCandUserChatList.length ; i ++) {
					...
					if (tmp.sourceUserId == lastMsg.sourceUserId) {
						lastestCandUserChatList.splice(i, 1);
						dealMsg = true;
						break;
					}
				}
			...
			cancelInterviewInvite(cancel_interviewId) {
				...
				// 保存最后一条消息到本地
				msgItem = this.dealMsgDateTime(msgItem);
				this.saveLastestMsgToLocal("[面试取消]", msgItem.chatTime);
				
				// 发送到server通信
				this.sendMSGToServer(this.MSGTYPE.MSG_INTERVIEW_CANCEL, cancel_interviewId, 0, "", null, null, null, null, null);
			...
			createInterviewInvite() {
				...
				uni.navigateTo({
					...
					url: "interview/createInterview?resumeUserId=" + resumeUserId,
			...
			showInterviewDetail(interviewId) {
				...
				var me = this;
				uni.navigateTo({
					url: "interview/interviewDetail?interviewId=" + interviewId + "&hrUserId=" + me.meInfo.id,
					animationType: "slide-in-bottom",
					success() {
					}
				});
			...
			sendInterviewInvite(interviewId) {
				...
				// 保存最后一条消息到本地
				this.saveLastestMsgToLocal("[面试邀约]", msgItem.chatTime);
				
				// 发送到server通信
				this.sendMSGToServer(this.MSGTYPE.INVITE, interviewId, msgItem.showMsgDateTimeFlag, "", null, null, null, null, null);
				...
				uni.removeStorageSync("choosenInterviewJob");
			},
			...
			enterFace(faceId) {
				...
				console.log(latestFaceList)
				console.log(faceId)
				console.log(typeof(latestFaceList));
			...
			sendMSGToServer(msgType, msg, showMsgDateTimeFlag, videoPath, videoWidth, videoHeight, videoTimes, voicePath, speakVoiceDuration) {
				var chatMsg = {
					...
					receiverId: this.resumeUserId,
					receiverType: 2,		// 此处，接受者为 求职者
					msg: msg,
					msgType: msgType,
