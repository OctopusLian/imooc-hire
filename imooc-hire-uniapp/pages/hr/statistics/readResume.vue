	...
	<view>
		<view class="resume-wrapper" v-for="(r, index) in allResumeList" :key="index" @click="gotoDetail(r.userId, r.resumeId, r.nickname, r.face, r.workYears, r.education, r.age, r.resumeExpectId)">
			...
			<view class="bottom-part" style="margin-top: 0px;">
				<text class="time-words">浏览时间</text>
				<text class="time-words" style="margin-left: 10px;">{{r.hrReadResumeTime}}</text>
			</view>
		</view>
		...
		methods: {
			...
			getPagedList(page) {
				...
				uni.request({
					...
					url: serverUrl + "/counts/pagedReadResumeRecordList?hrId=" + userId + "&page=" + page + "&limit=10",
					success(result) {
						console.log(result);
